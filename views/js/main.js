...
//----------Optimized----------
    function changePizzaSizes(size) {
        // switch statement to simplify which width has been chosen every case
        var newWidth;
        switch(size) {
            case "1":
                newWidth = 25;
        ...
        // for loop to set the width for every element to that percentage to resize the pizza in less than 5ms
...
function updatePositions() {
  ...
  var items = document.querySelectorAll('.mover');
    // for loop to generate some consistent pizzas across the screen with frame-rate at 60ps
  for (var i = 0; i < items.length; i++) {
    var phase = Math.sin((i / 1250) + (i % 5));
